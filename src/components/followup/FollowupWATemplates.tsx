...
import { Loader2, Plus, Trash2, MessageSquare, Info, Variable, Edit2, MousePointerClick } from 'lucide-react';
...
export interface VariableMapping {
  ...
  sub_type?: string; // 'url' | 'quick_reply' — only for buttons
  button_index?: number; // 0, 1, 2 — only for buttons
}
...
const FollowupWATemplates = () => {
  ...
  const addVariable = (component: string = 'body') => {
    // For body/header: auto-increment position within that component type
    ...
    if (component === 'button') {
      ...
      newVar.position = 1; // buttons usually have {{1}} for URL suffix
    }

    setNewTemplate(p => ({
      ...p,
      has_variables: true,
      variable_mappings: [...p.variable_mappings, newVar],
    }));
  };
  ...
  const removeVariable = (index: number) => {
    setNewTemplate(p => {
      ...
      // Re-number positions within each component group
  ...
  // Separate body/header vars from button vars for rendering
      ...
      <div className="bg-accent/50 border border-border rounded-lg p-3 flex gap-2">
        ...
        <div className="text-xs text-foreground/80 space-y-1">
          ...
          <ul className="list-disc list-inside space-y-0.5">
            ...
            <li>أزرار URL الديناميكية (مثلاً <code className="font-mono bg-muted px-1 rounded">{'example.com/track/{{1}}'}</code>) بتتربط بالبوليصة أو كود الطلب</li>
            <li>أزرار Quick Reply بتتبعت بالـ payload المحدد</li>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3 border-t border-border pt-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-1">
                  <Button type="button" variant="outline" size="sm" onClick={() => addVariable('body')} className="gap-1 text-xs">
                    <Plus className="w-3 h-3" />
                    Body
                  </Button>
                  <Button type="button" variant="outline" size="sm" onClick={() => addVariable('header')} className="gap-1 text-xs">
                    <Plus className="w-3 h-3" />
                    Header
                  </Button>
                </div>
              ...
              {bodyHeaderVars.length === 0 && (
                <p className="text-xs text-muted-foreground text-center py-1">لا توجد متغيرات نصية</p>
              )}
            ...
            <div className="space-y-3 border-t border-border pt-4">
              <div className="flex items-center justify-between">
                ...
                <Button type="button" variant="outline" size="sm" onClick={() => addVariable('button')} className="gap-1 text-xs">
                  <Plus className="w-3 h-3" />
                  إضافة زر
                </Button>
              </div>
              ...
              {buttonVars.length === 0 && (
                <p className="text-xs text-muted-foreground text-center py-1">لا توجد أزرار — لو القالب فيه أزرار بدون متغيرات مش محتاج تضيفها هنا</p>
              )}
                ...
                return (
                  <div key={index} className="p-2.5 bg-secondary/50 rounded-lg border border-border space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <Button type="button" variant="ghost" size="sm" className="h-6 w-6 p-0 text-destructive" onClick={() => removeVariable(index)}>
                        <Trash2 className="w-3 h-3" />
                      </Button>
                    </div>
                    ...
                    <div className="space-y-1">
                      <p className="text-[10px] text-muted-foreground">
                        {v.sub_type === 'quick_reply' ? 'الـ Payload (بيانات الرد)' : 'الجزء الديناميكي من الرابط {{1}}'}
                      </p>
                      <Select value={v.field} onValueChange={val => updateVariable(index, { field: val })}>
                        <SelectTrigger className="h-7 text-xs"><SelectValue /></SelectTrigger>
                        <SelectContent>
